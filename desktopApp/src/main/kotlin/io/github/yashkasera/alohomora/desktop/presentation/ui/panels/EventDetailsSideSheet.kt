...
fun EventDetailsSideSheet(
    ...
    AlohomoraSideSheet(
        ...
        event?.let { selected ->
            val isMuted = selected.name in uiState.filters.mutedNames
            Column(
                modifier = Modifier
                    .fillMaxWidth()
                ...
                KeyValueRow(label = "Name", value = selected.name, monospaceValue = true)
                KeyValueRow(
                    label = "Time",
                    value = DateUtils.format(selected.time, DateUtils.Format.ISO_DATE_TIME),
                    monospaceValue = true,
                )
                KeyValueRow(label = "Read", value = if (selected.isViewed) "Yes" else "No")

                // Solo lives here rather than on the name chips: a FilterChip's trailing icon is not
                // independently clickable, so in the chip row it would have to be a right-click menu
                // (a hidden gesture) or a second chip per name. Here it is a labelled button, next to
                // the name it acts on.
                Row(horizontalArrangement = Arrangement.spacedBy(MaterialTheme.dimens.margin.sm)) {
                    AlohomoraOutlinedButton(
                        ...
                        onClick = { eventsViewModel.toggleMute(selected.name) },
                    )
                    AlohomoraOutlinedButton(
                        text = "Show only this",
                        onClick = { eventsViewModel.soloName(selected.name) },
                    )
                }

                SectionLabel("Properties")
                // Scrollable here, unlike the row: the sheet is the one place the whole payload is
                // meant to be readable, and it owns its scroll rather than competing with a list.
                AlohomoraCodeBlock(content = properties, isScrollable = true)
            }
        }
    }
