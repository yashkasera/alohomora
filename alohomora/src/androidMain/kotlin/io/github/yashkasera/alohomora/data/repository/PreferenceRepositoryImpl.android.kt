...
/**
 * Android implementation of PreferenceRepository.
 *
...
internal class PreferenceRepositoryImpl(
    ...
    override suspend fun getStores(): List<PreferenceStore> = withContext(Dispatchers.IO) {
        val stores = mutableListOf<PreferenceStore>()
        ...
        if (prefsDir.exists() && prefsDir.isDirectory) {
            prefsDir.listFiles { file -> file.extension == "xml" }?.forEach { file ->
                ...
                val source = if (isEncrypted) {
                    PreferenceSource.ENCRYPTED_SHARED_PREFERENCES
                } else {
                    PreferenceSource.SHARED_PREFERENCES
                }
                ...
                stores.add(PreferenceStore(name, source, isEncrypted, count))
            }
        }
     ...
     */
    private suspend fun scanAllPreferences(): List<PreferenceEntry> = withContext(Dispatchers.IO) {
        val allEntries = mutableListOf<PreferenceEntry>()
     ...
     */
    private fun scanSharedPreferences(): List<PreferenceEntry> {
            ...
            try {
                ...
                entries.add(
                    PreferenceEntry(
                        ...
                        source = PreferenceSource.ENCRYPTED_SHARED_PREFERENCES,
                        isEncrypted = true,
                        storeName = storeName,
                    )
                )
