...
/**
 * Holds the single [TraceReplayHandler] the host app registered, and runs replays through it.
 *
 ...
 */
internal object TraceReplayRegistry {

    @Volatile
    private var handler: TraceReplayHandler? = null

    /** True when the host app has supplied a handler, so replay can be offered at all. */
    val isSupported: Boolean get() = handler != null

    fun register(handler: TraceReplayHandler) {
